...
- `scripts/herddesk_g0` — Python strict JSON, frame/input checks, NDJSON, capture validator, endpoint matrix, lease matrix, renderer L1 matrix, ADR baseline, project graph, HD-033 Narrator overlay, HD-033 product-UI Narrator launch record check, HD-033 current-system-DPI overlay, HD-033 current-system theme overlay, HD-033 product-UI soak START check, HD-033 soak START interruption check, HD-033 wall-clock elapsed overlay check, HD-033 optional soak-process working-set overlay check, HD-033 optional scale-only DPI matrix overlay check.
...
- [ ] `herddesk_g0.release` keeps `ac40_passed`, `published`, and `complete_1_0_claimed` false. A hosted workflow run is not a required-check. `github_required_check` stays UNVERIFIED. A git HEAD mismatch with the bound SHA is reported and does not pass AC40. A matching HEAD also does not pass AC40.
- [ ] `herddesk_g0.quality` keeps `ac37_passed`, `live_narrator`, `ac38_passed`, `live_dpi`, `ac46_passed`, `live_soak`, `ac29_passed`, and `live_working_set` false on catalog/START/L2. Catalog/START/L2 keep `eight_hour_soak_executed` false. Narrator.exe presence is not screen-reader evidence. The Narrator overlay does not start Narrator and does not pass AC37. A product-UI `--ui` + Narrator.exe launch record is not `live_narrator` success and does not pass AC37. The current-system-DPI overlay does not change display scale, is not a 100/150/200 matrix, and does not pass AC38. The current-system theme overlay does not change AppsUseLightTheme, high-contrast, or display topology, is not a light/dark/high-contrast x monitor matrix, and does not pass AC38. A committed scale-only `live-dpi-matrix.json` may set `dpi_matrix_100_150_200_executed` true on that file only after 100/150/200 were applied and original scale restored; it is not AC38, not theme/monitor/high-contrast, not L3, and does not pass `live_dpi`. Pointer overlay and `live-dpi.not-run.json` keep matrix flags false. A product-UI `--ui` soak START is not `eight_hour_soak_executed`, not `soak_hours=8`, and does not pass AC46. Interrupted STARTs are not 8h completion. A later START must be a new wall-clock and must not reuse interrupted owned PIDs. `--record-elapsed` fails closed before 8 wall-clock hours or when START PIDs do not match live processes; `--watch-elapsed` spawns a detached due-watcher and must not add its PID to START owned_pids; a committed `live-soak-elapsed.json` may set `eight_hour_soak_executed` true on that file only and is not AC46. A soak-process working-set overlay of the running START App PID is optional until `--record`; it is not 1/4 visible pane, not 100 open/close, not `live_working_set` success, and does not pass AC29 or AC46. `live-working-set.not-run.json` stays the 1/4-pane lab not_run row.
