...
生成日期：2026-09-08。G0 领域逻辑标本：单连接帧解析、输入放行策略、endpoint 纯映射、终端 lease 观测映射、HD-005 renderer L1 纯函数标本、HD-009 内存投影 Store、HD-010 每 SessionKey 一个 `DeviceSession` actor、HD-012 L1 注意力 reducer / 未读聚合、HD-016 L1 `ControlLeaseCoordinator`、HD-017 L1 `ResourceCommandCoordinator`、HD-018 L1 `RecoveryPolicy` 与跨 owner 恢复信号、HD-023 L1 多设备 partition 聚合、HD-024 L1 每 SessionKey 退避与认证/host-key 阻断（无第二 RecoveryManager），HD-025 L1 `ConnectionAdmissionPolicy` / `TerminalQueueBudget` / `DirtySetBudget`，HD-028 L1 `TransferCoordinator` / `TransferStateMachine`（无 SSH），HD-030 L1 `AttachmentCoordinator` / `AttachmentCapabilityCatalog`（无 WinUI、无 live agent），以及 HD-031 L1 `ClipboardIntentResolver` / `PasteCoordinator`（无 WinUI、无 live clipboard watcher）。
...
[../../tests/HerdDesk.Core.SmokeTests](../../tests/HerdDesk.Core.SmokeTests/CLAUDE.md) 覆盖解析、策略、endpoint resolver 与 lease mapper 断言。[../../tests/Unit/HerdDesk.Core.Tests](../../tests/Unit/HerdDesk.Core.Tests/CLAUDE.md) 覆盖 InputPolicy、parser latch、Core 程序集边界、HD-009 mapper/Store（fake decoded 输入）、HD-010 DeviceSession L1 race（fake RPC ports、barrier，不用 sleep）、HD-012 Attention reducer（基线/去重/未读/stale）、HD-016 ControlLeaseCoordinator（fake HD-013 事件）、HD-017 ResourceCommandCoordinator（fake RPC/Store），HD-018 RecoveryPolicy/DeviceSession/lease 恢复（barrier，fake TimeProvider），HD-019 L1 composition（DeviceSession + lease + InputPolicy + RecoveryPolicy + ResourceCommand + Attention，fakes），HD-023 L1 多设备 partition 隔离、resolver、搜索 p95 与旧 epoch 并发（in-memory fake Store），HD-028 L1 TransferCoordinator（fake IFileEndpoint），HD-030 L1 AttachmentCoordinator（capability/state/target race/zero-enter，fake ports），以及 HD-031 L1 clipboard intent/paste（fake snapshot，无 live clipboard）。Python 侧有对等意图的校验器，见 [../../scripts/CLAUDE.md](../../scripts/CLAUDE.md)。两套实现未自动生成，不能互相替代。计数以本次 `dotnet run` 为准。L2 live subscribe interleave 为 UNVERIFIED，见 `implementation/hd-010-l2.json`。HD-012 Windows toast L2 为 UNVERIFIED，见 `implementation/hd-012-l2.json`。HD-016 L2 live lease 为 UNVERIFIED，见 `implementation/hd-016-l2.json`。HD-017 L2 live mutation 为 UNVERIFIED，见 `implementation/hd-017-l2.json`。HD-018 L2 live disconnect 为 UNVERIFIED，见 `implementation/hd-018-l2.json`。HD-019 L2 live local MVP 与 L3 IME/TUI 为 UNVERIFIED，见 `implementation/hd-019-l2.json` 与 `implementation/hd-019-l3.json`。
...
- `Files/` — HD-028 L1 `TransferCoordinator` / `TransferStateMachine`。Local↔Local 走 `IFileEndpoint`；复用 HD-025 file lease。L2 live FS/SSH UNVERIFIED。
- `Attachments/` — HD-030 L1 `AttachmentCapabilityCatalog` / `AttachmentCoordinator`。三种明确意图、证据化 capability、不可变 draft 与 target lease、HD-028 hash+rename 完成后经 `InputPolicy` 路径输入且不附加 Enter。成功只到 `PathInsertedUnconfirmed`。L2 live agent/IME/SSH UNVERIFIED。
- `Clipboard/` — HD-031 L1 `ClipboardIntentResolver` / `PasteCoordinator`。显式 snapshot 才读取；`Text|FileList|Image|Mixed|Empty|Unsupported|AccessDenied`；多行确认默认 Cancel；Confirm 发送原 bytes 且 0 个额外 Enter；目标 lease 冻结 PaneKey+epoch+control。不注册 clipboard watcher。L2 live clipboard/IME UNVERIFIED。
