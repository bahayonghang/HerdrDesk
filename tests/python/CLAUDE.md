...
| `test_hd032.py` | `Hd032ResidualTests` | `implementation/hd-032-l2.json` 与 `evidence/files/` 保持 L2 live FS/SSH/TOCTOU/attack UNVERIFIED；AC31/32/33/34/35/G0 恒为 false；模板不是成功运行；无 `tests/Integration.Ssh` / `Integration.Windows` |
| `test_hd033.py` | `Hd033ResidualTests` | `implementation/hd-033-l2.json` 与 `evidence/quality/` 保持 L3 IME/Narrator/DPI 与 L4 soak UNVERIFIED；AC27/28/29/37/38/46/G0 恒为 false；拒绝假 soak 与发明 timings；parser consumed 不是呈现；无 `tests/Integration.Windows` |
| `test_hd034.py` | `Hd034ResidualTests` | `implementation/hd-034-l2.json` 与 `evidence/packaging/` 保持 L2/L3 live 安装/签名/更新/回滚 UNVERIFIED；AC41/AC42/G0 恒为 false；拒绝假 Publisher、未签名构建当发行安装、EXE 复制当回滚、发明包 hash；无 `tests/Integration.Windows` |
| `test_hd035.py` | `Hd035ResidualTests` | `implementation/hd-035-l2.json` 与 `evidence/security-release/` 保持 L2/L3 live 扫描/renderer/canary/已签名包反向审计 UNVERIFIED；AC02/AC43/AC44/G0 恒为 false；拒绝假零漏洞扫描、公开可见当许可、herdrm 拷贝、发明扫描日期当成功；无 `tests/Integration.Windows` |
| `test_hd036.py` | `Hd036ResidualTests` | `implementation/hd-036-l2.json` 与 `evidence/releases/` 保持 L2/L3/L4 live 走查/矩阵/SHA 绑定/干净还原/required-check/图重跑/发布/签名 hash UNVERIFIED；AC39/AC40/AC45/AC47/AC48/G0 恒为 false；拒绝 published、完整 1.0 宣称、发明 hosted-check、截图当证据、not_run→passed；无 `tests/Integration.Windows` |
...
| `test_project_graph.py` | `ProjectGraphTests` | 允许边通过；App 准入 WinUI 2.3.6；Core/Contracts/随机包、umbrella WASDK、Core 的 lock、额外 NuGet 源均失败。`bridge/` 不在 C# 项目图内。`Cargo.lock` 的 `interprocess` 版本必须与 `implementation/hd-008-packages.json` 一致；L2 保持 UNVERIFIED |
