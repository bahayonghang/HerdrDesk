...
| `test_hd033.py` | `Hd033ResidualTests`, `Hd033NarratorOverlayTests`, `Hd033DpiOverlayTests` | `implementation/hd-033-l2.json` 与 `evidence/quality/` 保持 L3 IME/Narrator/DPI 与 L4 soak UNVERIFIED；调用已交付 `collect_narrator_overlay` / `scripts/collect_narrator_overlay.py` 与 `collect_dpi_overlay` / `scripts/collect_dpi_overlay.py`；Narrator overlay 不是 AC37；DPI overlay 不是 AC38，也不是 100/150/200 矩阵；AC27/28/29/37/38/46/G0 恒为 false；拒绝假 soak 与发明 timings；parser consumed 不是呈现；`integration_windows_project` catalog token 恒为 false |
...
- 代码：`scripts/herddesk_g0`（含 `evidence.py`、`licensing.py`、`quality.py`、`release.py`、`endpoint.py`、`lease.py`、`renderer.py`、`adr.py`、`project_graph.py`）、`probe_herdr.py`、`publish_github.py`、`validate_repository.py`、`collect_narrator_overlay.py`、`collect_dpi_overlay.py`、`audit_release_inputs.py`、`bind_release_candidate.py`。
