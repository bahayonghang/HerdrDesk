...
回归 Python 协议、探针安全门、发布清单、仓库可移植性、许可台账、endpoint 矩阵。不执行 herdr，不启动 GUI。
...
- 代码：`scripts/herddesk_g0`（含 `evidence.py`、`licensing.py`、`endpoint.py`）、`probe_herdr.py`、`publish_github.py`、`validate_repository.py`。
