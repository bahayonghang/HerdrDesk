...
| [testing/performance-soak.md](testing/performance-soak.md) | HD-033 L1 收口说明加 L2 Narrator overlay、产品 UI `--ui` + Narrator.exe 启动记录、L2 当前系统 DPI overlay，产品 UI `--ui` soak START，与 soak START 中断记录。AC27/28/29/37/38/46 为 `not_run`。Narrator overlay 不是 AC37。启动记录不是 AC37。DPI overlay 不是 AC38，也不是 100/150/200 矩阵。soak START 不是 AC46。中断不是 8h。不是 live soak / 输入到像素 / DPI / Narrator 通过。 |
