...
`phase_gate=not_passed`。`g0_passed=false`。`ac41_passed` / `ac42_passed` 为 false。模板文件 `template=true`，空 hash / 退出码 / Publisher，不是一次成功运行。假 Publisher 不能让 AC41 通过。未签名本地构建不能当发行安装证据。复制旧 EXE 不能当回滚。`packaging/` 与 `tests/Integration.Windows` 未建仓。
...
残差 JSON：`implementation/hd-034-l2.json`。`l2_live_install` / `l2_live_sign` / `l2_live_update` / `l2_live_rollback` / `l3_clean_machine` 为 `UNVERIFIED`。产品 AC41/AC42 仍为 `not_run`。`tests/Integration.Windows` 不存在。
