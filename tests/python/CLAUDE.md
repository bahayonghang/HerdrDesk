...
| `test_hd033.py` | `Hd033ResidualTests`, `Hd033NarratorOverlayTests`, `Hd033NarratorProductUiLaunchTests`, `Hd033DpiOverlayTests`, `Hd033DpiMatrixTests`, `Hd033SoakStartTests`, `Hd033SoakElapsedTests`, `Hd033SoakInterruptionTests`, `Hd033SoakWorkingSetTests` | `implementation/hd-033-l2.json` 与 `evidence/quality/` 保持 L3 IME/Narrator/DPI 与 L4 soak UNVERIFIED；调用已交付 `collect_narrator_overlay` / `scripts/collect_narrator_overlay.py`、`validate_narrator_product_ui_launch` / `scripts/record_narrator_product_ui_launch.py`（默认不 `--record`）、`collect_dpi_overlay` / `scripts/collect_dpi_overlay.py`、`validate_dpi_matrix` / `scripts/record_dpi_matrix.py`（默认不 `--record`；hooks 测试不得改开发机缩放；scale-only overlay 不是 AC38，不是 theme/monitor/high-contrast）与 `validate_eight_hour_soak_start` / `validate_eight_hour_soak_interruption` / `validate_eight_hour_soak_elapsed` / `scripts/start_eight_hour_soak.py`（默认不 `--record` / `--record-elapsed` / `--watch-elapsed`；过早 `--record-elapsed` 与 tempfile watcher/`record_elapsed` fail-closed，不写成功 `live-soak-elapsed.json`；已提交墙钟 elapsed 身份钉在 `Hd033SoakElapsedTests`；`--watch-elapsed` 不把 watcher PID 写入 START `owned_pids`）与 `validate_soak_working_set` / `scripts/record_soak_working_set.py`（默认不 `--record`；overlay 文件可不存在；不冻结当前 START 为 76508/85848）；Narrator overlay 不是 AC37；产品 UI 启动记录不是 `live_narrator` 成功，也不是 AC37；DPI overlay 不是 AC38，也不是 100/150/200 矩阵；soak START 不是 `eight_hour_soak_executed`，也不是 AC46；中断的 START 不是 8h 完成；当前 START 不得冻结为 PID 46108/64672/71980 或 24744/27844 或 21312/22400 或 91852/37708 或 69668/73960；elapsed 文件可不存在；soak 进程 working-set overlay 不是 AC29、不是 1/4 pane、不是 100 次 open/close；`live-working-set.not-run.json` 保持 not_run；AC27/28/29/37/38/46/G0 恒为 false；拒绝假 soak 与发明 timings；parser consumed 不是呈现；`integration_windows_project` catalog token 恒为 false |
