...
HD-027 `FileBridgeProtocolException` / Rust `herddesk_filebridge::Error` messages are the code only (`payload_too_large`, `protocol_pollution`, `replace_observation_required`, `sequence_wrap`, …). Unknown ErrorJson codes stay opaque. L2 FS/SSH/TOCTOU remain UNVERIFIED. Product AC30/AC34 stay not passed.
