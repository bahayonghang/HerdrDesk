...
| `licensing.audit_admitted_release_inputs` | HD-035 L2 工作树 admitted-input auditor；核对 App/npm/Cargo lock 存在与台账对齐；不是 live 扫描；AC02/AC43/AC44 恒为 false |
...
| `quality.collect_theme_overlay` | HD-033 L2 当前系统 theme overlay；记录 AppsUseLightTheme、SystemUsesLightTheme、HighContrast、monitor count；不改主题/高对比/拓扑；单样本不是 light/dark/high-contrast × 显示器矩阵；AC38 恒为 false |
...
| CLI `scripts/record_lab_msix.py` | HD-034 可选 lab MakeAppx/SignTool overlay；默认只校验；`--record` 不是 CI；`lab_msix_packed` / `lab_signature_applied` 仅可在 `live-lab-msix.json` 为 true；`signed_msix_built` / AC41 恒为 false |
...
| `new_lab_certificate.ps1` | HD-034 L2 lab CurrentUser 证书 overlay；stdout 一个 JSON 对象。PFX 只写调用方路径（默认 gitignored `artifacts/`）。禁止写入 `packaging/` 或源树。不是 AC41 |
| `collect_narrator_overlay.py` | HD-033 L2 Narrator 存在性 overlay CLI；stdout 一个 JSON 对象。不启动 Narrator。不是 AC37 |
| `record_narrator_product_ui_launch.py` | HD-033 产品 UI `--ui` + Narrator.exe 启动记录。默认只校验已提交 JSON。`--record` 仅交互桌面；CI/`just` 不得调用。前台重试 + Ctrl+K 不是 AC37 工作流完成。不是 AC37 |
| `collect_dpi_overlay.py` | HD-033 L2 当前系统 DPI overlay CLI；stdout 一个 JSON 对象。不改显示缩放。不是 AC38，也不是 100/150/200 矩阵 |
| `collect_theme_overlay.py` | HD-033 L2 当前系统 theme overlay CLI；stdout 一个 JSON 对象。不改主题/高对比/拓扑。不是 AC38，也不是 light/dark/high-contrast × 显示器矩阵 |
...
| `bind_release_candidate.py` | HD-036 L2 hosted-workflow pointer CLI；stdout 一个 JSON 对象。hosted workflow 不是 required-check。AC39/AC40/AC45/AC47/AC48 仍为 false。不是完整 1.0 |
| `record_lab_msix.py` | HD-034 可选 lab MakeAppx/SignTool overlay。默认只校验可选 JSON。`--record` 仅交互桌面创建 gitignored lab PFX、MakeAppx pack、SignTool sign；CI/`just` 不得调用。`lab_msix_packed` / `lab_signature_applied` 仅可在 `live-lab-msix.json` 为 true。不是 AC41，不是 live 安装，不是发行 Publisher |
