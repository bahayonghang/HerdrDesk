...
HD-034 L2 unsigned local layout. Not a WAP project. App stays `WindowsPackageType=None`. Live install/sign/update/rollback stay UNVERIFIED. AC41/AC42/G0 stay not passed.
...
- 构建入口是 `scripts/package_release.ps1`。默认产物在 gitignored `artifacts/packaging/`。未签名本地构建不能当发行安装。
