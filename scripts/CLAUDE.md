...
`validate_repository.py` 另读取 `implementation/hd-019-l2.json`、`implementation/hd-019-l3.json`、`implementation/hd-020-l2.json`、`implementation/hd-021-l2.json`、`implementation/hd-022-l2.json`、`implementation/hd-023-l2.json`、`implementation/hd-024-l2.json` 与 `evidence/local-mvp/catalog.json`。L2/L3 live local MVP 保持 `UNVERIFIED`。L2 live helper deploy 保持 `UNVERIFIED`。L2 live SSH 保持 `UNVERIFIED`。L2 live auth 保持 `UNVERIFIED`。产品 AC06/AC07/AC10/AC15/AC25/AC24/AC26 与 `g0_passed` 保持 false。`tests/Integration.Windows` 与 `tests/Integration.Ssh` 不得存在。
