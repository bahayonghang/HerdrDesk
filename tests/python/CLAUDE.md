...
| `test_hd033.py` | `Hd033ResidualTests`, `Hd033NarratorOverlayTests`, `Hd033NarratorProductUiLaunchTests`, `Hd033DpiOverlayTests`, `Hd033SoakStartTests` | `implementation/hd-033-l2.json` 与 `evidence/quality/` 保持 L3 IME/Narrator/DPI 与 L4 soak UNVERIFIED；调用已交付 `collect_narrator_overlay` / `scripts/collect_narrator_overlay.py`、`validate_narrator_product_ui_launch` / `scripts/record_narrator_product_ui_launch.py`（默认不 `--record`）、`collect_dpi_overlay` / `scripts/collect_dpi_overlay.py` 与 `validate_eight_hour_soak_start` / `scripts/start_eight_hour_soak.py`（默认不 `--record`）；Narrator overlay 不是 AC37；产品 UI 启动记录不是 `live_narrator` 成功，也不是 AC37；DPI overlay 不是 AC38，也不是 100/150/200 矩阵；soak START 不是 `eight_hour_soak_executed`，也不是 AC46；AC27/28/29/37/38/46/G0 恒为 false；拒绝假 soak 与发明 timings；parser consumed 不是呈现；`integration_windows_project` catalog token 恒为 false |
