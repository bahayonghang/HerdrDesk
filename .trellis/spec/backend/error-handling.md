...
HD-031 clipboard codes include `clipboard_read_denied`, `clipboard_write_denied`, `mixed_intent_required`, `multiline_confirm_required`, `cache_full`, `cleanup_failed`, `access_denied`, `busy`, `oversize`, and `target_stale`. OSC 52 parser/policy does not call the snapshot reader. Cancel sends 0 bytes. Confirm sends original bytes with no extra Enter. Diagnostics omit clipboard text/image/file content, full paths, and terminal body. L2 live clipboard/IME remain UNVERIFIED. Product AC36 stays not passed.
