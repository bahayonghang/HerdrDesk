...
HD-033 L1 P5 性能/可访问性/soak 收口目录，外加 L2 采集器指针。`catalog.json` 列出 cold start / input-to-visible-pixel / search p95 / working set 1/4 pane / 100 hide-show / Narrator / DPI 100-150-200 / 8h soak 执行卡。`environment-pointer.json` 指向 gitignored `probe-results/` 原始环境清单，live 仍 `not_run`。`narrator-overlay-pointer.json` 只记录 Narrator.exe 存在与否；不是 AC37，已交付 AutomationProperties 名称不是屏幕阅读器证据。`dpi-overlay-pointer.json` 只记录当前系统 DPI；不是 AC38，也不是 100/150/200 矩阵。scale-only 矩阵 overlay 见 `evidence/quality/live-dpi-matrix.json`；仅该文件可将 `dpi_matrix_100_150_200_executed` 置 true，存在也不是 AC38，不是 theme/monitor/high-contrast，不是 L3。`live-soak-start.json` 只记录产品 UI `--ui` soak START；不是 `eight_hour_soak_executed`，不是 AC46。`--record-elapsed` 在墙钟未满 8h 或 START PID 不匹配时 fail-closed，不写成功 `live-soak-elapsed.json`。已提交的墙钟 elapsed 见 `evidence/quality/live-soak-elapsed.json`；仅该文件可将 `eight_hour_soak_executed` 置 true，存在也不是 AC46。`--watch-elapsed` 拉起 detached due-watcher，到期才写 elapsed；watcher PID 不写入 START `owned_pids`；到期前不写 elapsed。`live-soak-working-set.json` 可选；存在时是已运行 START App PID 的 soak 进程样本，不是 1/4 pane、不是 100 次 open/close、不是 `live_working_set` 成功、不是 AC29/AC46。`live-working-set.not-run.json` 仍是 1/4 pane lab 的 not_run 行。`live-soak-interrupted.json` 记录 2026-09-10T09:49:06Z START 在 8h 前退出；`live-soak-interrupted-2.json` 记录 2026-09-10T11:11:22Z START 在 8h 前退出；`live-soak-interrupted-3.json` 记录 2026-09-10T11:35:26Z START 在 8h 前退出（HerdDesk.App.exe PID 24744 died；heartbeat last alive 11:46:44Z；alive_pids=[] at 11:47:45Z；heartbeat python 27844 leftover；crash_cause null）；`live-soak-interrupted-4.json` 记录 2026-09-10T11:56:07Z START 在 8h 前退出（PIDs 21312/22400；heartbeat last alive 11:57:20Z；alive_pids=[] at 11:58:21Z；crash_cause null；soak_hours null）；`live-soak-interrupted-5.json` 记录 2026-09-10T12:19:50Z START 在 8h 前退出（PIDs 91852/37708；heartbeat last alive 12:20:04Z；alive_pids=[] at 12:21:05Z；crash_cause null；soak_hours null）；`live-soak-interrupted-6.json` 记录 2026-09-10T12:49:43Z START 在 8h 前退出（PIDs 69668/73960；heartbeat last alive 13:05:06Z；alive_pids=[] at 13:06:07Z；crash_cause null；soak_hours null）。中断不是 8h 完成，也不是 AC46。当前 START 必须晚于这些中断墙钟且 PID 不相交，不得冻结为 21312/22400 或 91852/37708 或 69668/73960。parser consumed 不是 GPU 或可见像素呈现。不得从 `Q_p` 推导进程工作集。MiB = 1,048,576 bytes。每个 live 行 `status=UNVERIFIED`、`result=not_run`。`github_required_check` 为 `UNVERIFIED`。`support-matrix.json` 承诺 Windows 11 x64 客户端 + Linux x64 远端，两行均为 `not_run`。macOS/ARM64 为 `unsupported` 或 `experimental`。模板 `template=true`，空 hash/退出码，不是成功运行。不得发明 timings。hosted CI 不是交互桌面。产品 AC27/AC28/AC29/AC37/AC38/AC46 仍未通过。

## `packaging/`

HD-034 L2 P5 签名/安装/更新回滚收口目录，外加未签名 lab layout 叠加、lab 证书 overlay，以及可选 lab MakeAppx/SignTool overlay。`catalog.json` 列出 clean install / runtime missing / signed update / bad publisher-or-tamper / signed rollback / config backup-restore / file-job defer / unsigned local build 执行卡。`lab-sign-overlay-pointer.json` 只记录 `scripts/new_lab_certificate.ps1` 存在；不是 live 签名成功行，不是 AC41。`lab_msix_capture` 指向可选 `live-lab-msix.json`；该文件可将 `lab_msix_packed` / `lab_signature_applied` 置 true，但 `signed_msix_built` / `live_sign` / AC41 仍为 false。PFX 与 lab MSIX 保持 gitignored。`App.xaml` 已存在。`packaging/` 是 lab identity 未签名 layout，不是 WAP 工程，不是发行 Publisher。假 Publisher 不能让 AC41 通过。未签名本地构建不能当发行安装。复制旧 EXE 不能当回滚。每个 live 行 `status=UNVERIFIED`、`result=not_run`。`support-matrix.json` 承诺 Windows 11 x64 客户端，行为 `not_run`。Linux x64 是远端 OS，不是 MSIX 客户端。macOS/ARM64 为 `unsupported` 或 `experimental`。模板 `template=true`，空 hash/退出码/Publisher，不是成功运行。产品 AC41/AC42 仍未通过。
...
HD-036 L1 P5 发布文档与追踪归档收口目录，外加 L2 hosted-workflow pointer overlay。`catalog.json` 列出 user-guide / support-matrix / ac48-trace / ac39-clean-restore / ac40-hosted-required-check / ac47-dep-graph / unpublished-candidate / signed-hash-sbom 执行卡，指向已有 HD-001–035 L1 产物与 `docs/user-guide/`、`docs/release/`。`hosted-workflow-pointer.json` 绑定 observed SHA `d834752` 与 hosted run `34549580783`；hosted workflow 不是 GitHub required-check ruleset。`ac-index.json` 追溯 AC01–AC48，状态保持 `not_run`。每个 live 行 `status=UNVERIFIED`、`result=not_run`。`candidate_sha` 与 `hosted_check_run_id` 保持 null。`support-matrix.json` 承诺 Windows 11 x64 客户端 + Linux x64 远端，两行均为 `not_run`。macOS/ARM64 为 `unsupported` 或 `experimental`。core 1.0 不冒充 EP-01..EP-07。模板 `template=true`，空 hash，不是成功运行。未发布。不是完整 1.0。产品 AC39/AC40/AC45/AC47/AC48 仍未通过。
