...
filebridge/                 HD-027 L1 protocol codec + spec/test-vectors (no main.rs, no FS)
...
Present: `src/HerdDesk.App` (BCL host + HD-011 ViewModels + HD-015 focus/input ViewModels + HD-016 control ViewModel + HD-017 resource command ViewModel + HD-018 RecoveryBindings + HD-023 L1 multi-device navigation/search ViewModels), `src/HerdDesk.Infrastructure` (including HD-027 L1 `Files/` codec), `src/HerdDesk.Terminal.Web` (HD-014 L1 adapter + HD-015 L1 coordinators), `bridge/herddesk-bridge`, `filebridge/` L1 codec (no binary). Planned and **not** present: `src/HerdDesk.Terminal.Native`, WinUI XAML shell, `herddesk-filebridge` serve binary, `tests/Integration.Windows`. L2 named-pipe ACL is UNVERIFIED. HD-011 L2 visual/activation and L3 IME/DPI are UNVERIFIED. HD-014 L2 WebView process and L3 DPI are UNVERIFIED. HD-015 L3 real IME desktop is UNVERIFIED. HD-016 L2 live lease is UNVERIFIED. HD-017 L2 live mutation is UNVERIFIED. HD-018 L2 live disconnect is UNVERIFIED. HD-023 L2 live 3-device p95 is UNVERIFIED.
