...
| `quality.validate_eight_hour_soak_start` | HD-033 产品 UI `--ui` 8h soak START 记录校验；不启动进程；不是 AC46；`eight_hour_soak_executed` 与 `live_soak` 恒为 false |
...
| `start_eight_hour_soak.py` | HD-033 产品 UI `--ui` 8h soak START。默认只校验已提交 JSON。`--record` 仅交互桌面；CI/`just` 不得调用。不是 AC46 |
